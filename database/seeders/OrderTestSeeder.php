...
// السر الأول: تسريع عمليات الإدخال الضخمة وحمايتها عبر الترانزاكشن
DB::transaction(function () use ($products) {
    $hashedPassword = Hash::make('password123');

    for ($i = 1; $i <= 10000; $i++) {
        
        // 1. إنشاء المستخدم بحقل first_name الصحيح
        $user = User::create([
            'first_name' => "Test User $i", 
            'email'      => "user{$i}@test.com", 
        ...
        // 2. إنشاء الطلب - متوافق 100% مع الميجريشن بدون حقول أسعار
        $order = Order::create([
            'user_id' => $user->id,
            'status'  => 'pending', // حالته معلقة بانتظار فحص التيستر
        ]);

        // اختيار منتج عشوائي لتحديد سعره لعناصر الطلب
        $randomProduct = $products->random();

        // 3. إنشاء عناصر الطلب - متوافق 100% مع ميجريشن order__items الخاص بكِ
        Order_Item::create([
            'order_id'    => $order->id,
            'product_id'  => $randomProduct->id,
            'price'       => $randomProduct->price,       // سعر القطعة الواحدة
            'amount'      => 1,                           // الكمية
            'total_price' => $randomProduct->price,       // السعر الإجمالي للعنصر
        ]);
    }
});
