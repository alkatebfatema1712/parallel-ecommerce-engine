...
use App\Http\Controllers\ParallelProjectController;
...
Route::middleware(['auth:sanctum', 'throttle:checkout-capacity', 'idempotency'])->group(function () {

    Route::post('/order/transfer', [OrderController::class, 'transfer_cart_contents']);

    Route::post('/order/{orderId}/confirm', [OrderController::class, 'confirm_order']);
});
//*********************************************************************************************************
...
/****************************************************************************************************/
// Parallel Programming Project - Final Technical Endpoints
Route::middleware(['performance.monitor'])->group(function () {
    Route::get('/parallel/products/popular', [ParallelProjectController::class, 'popularProducts']);
    Route::get('/parallel/products/{id}/cached', [ParallelProjectController::class, 'productCached']);
    Route::get('/parallel/search', [ParallelProjectController::class, 'searchCached']);
    Route::get('/parallel/resource-heavy', [ParallelProjectController::class, 'resourceHeavy']);
    Route::post('/parallel/batch/daily-sales', [ParallelProjectController::class, 'dailyBatchReport']);
    Route::get('/parallel/metrics', [ParallelProjectController::class, 'metrics']);
    Route::post('/parallel/lock-demo/{productId}', [ParallelProjectController::class, 'lockDemo']);

    Route::middleware(['auth:sanctum', 'throttle:checkout-capacity'])->group(function () {
        Route::post('/parallel/checkout/{orderId}', [ParallelProjectController::class, 'checkout']);
    });
});
